...
def write_to_sqlite():
    ...
    with Session() as session:
        ...
        for index, row in df.iterrows():
            gender = session.query(Gender).filter_by(gender=row['gender']).first()
            customer_type = session.query(CustomerType).filter_by(customer_type=row['customer_type']).first()
            type_of_travel = session.query(TravelType).filter_by(travel_type=row['type_of_travel']).first()
            class_type = session.query(ClassType).filter_by(class_type=row['class']).first()
